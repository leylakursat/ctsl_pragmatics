...
ggsave(file="../graphs/proportion_language.pdf",width=8,height=5)
```
...
ggplot(agr, aes(x=condition,y=Mean,fill=speakerName, alpha=condition)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = Ymin, ymax = Ymax),width=.25) +
  xlab("Trial type") +
  facet_wrap(speakerName ~ MentionType, nrow=15) + 
...
ggsave(file="../graphs/by_participant.pdf",width=8,height=30)
```
...
toplot = toplot %>%
  ...
  mutate(uniquePos = trimws(uniquePos,which=c("left"))) %>%
