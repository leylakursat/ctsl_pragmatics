        ...
        if key != "trials":
            ...
            for j in range(len(line)):
                if line[j] == "{":
                    ...
                    end_bracket = j + 1
                    new_line = line[start_bracket:end_bracket].replace('\\"','').replace('\\','')
            ...
            for j in range(len(line)):
                if line[j] == "{":
                    ...
                    end_bracket = j + 1
                    new_line = line[start_bracket:end_bracket].replace("false", "False").replace("true", "True")
                    new_line = ast.literal_eval(new_line)
                    new_lines.append(new_line)
                    #print("new_line: ",new_line)
            new_lines.pop(0) 
            new_data[key] = new_lines
            print("new_lines: ", new_lines[0])
    return new_data
...
def make_header(dataframe):
    ...
    print("trials_header: ", trials_header)
    ...
    # print("catch_trials_header: ", catch_trials_header)
    ...
    print("subj_header: ", subj_header)
    for label in subj_header:
        header.append(label)
    time_in_minutes = {}

    # if "time_in_minutes" in dataframe:
    #     time_in_minutes = dataframe["time_in_minutes"]
    # time_in_minutes_header = time_in_minutes[0].keys()
    # print("time_in_minutes_header: ", time_in_minutes_header)
    # for label in time_in_minutes_header:
    #     header.append(label)
    print("HEADER: ", header)
    return header
