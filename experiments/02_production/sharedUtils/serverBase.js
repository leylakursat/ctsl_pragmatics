...
module.exports = function (expName) {
  ...
  var serverLocal = require('../' + expName + '/experiment/game.server.js');
  ...
  var core = require('../' + expName + '/experiment/game.core.js');

  // This is the important function that pairs people up into 'rooms'
  // all independent of one another.
  gameServer.findGame = function (query, player) {
    ...
    if (!joined_a_game) {
      this.createGame(query, player);
    }
  };

  // Will run when first player connects
  gameServer.createGame = function (query, player) {
