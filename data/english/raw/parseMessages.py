...
csv_messagenames = [o for o in os.listdir("./message/") if (o.endswith('csv') & o.startswith('2021-41'))]
csv_trialnames =  [o for o in os.listdir("./clickedObj/") if (o.endswith('csv') & o.startswith('2021-41'))]
...
for k,m in enumerate(csv_messagenames):
	...
	messagereader = csv.DictReader(open(datadir+"/message/"+m, 'rt'),delimiter=",",quotechar='\"')
	messagelines.extend(list(messagereader))

	trialreader = csv.DictReader(open(datadir+"/clickedObj/"+m, 'rt'),delimiter=",",quotechar='\"')
...
w = csv.DictWriter(open("merged_data.csv", "w"),fieldnames=headers,restval="NA",delimiter="\t")
w.writeheader()
